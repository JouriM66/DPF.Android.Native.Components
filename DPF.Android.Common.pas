...
function GetAppFolder: string;
begin
  result := IncludeTrailingBackslash( JStringToString( TAndroidHelper.Activity.getApplicationContext.getFilesDir.getAbsolutePath ) );
end;
...
function CheckPermission( const permissionName: string ): boolean;
begin
  Result := TAndroidHelper.Context.checkCallingOrSelfPermission( StringToJString( permissionName ) ) = TJPackageManager.JavaClass.PERMISSION_GRANTED;
end;
...
begin
  try
    obj := TAndroidHelper.Context.getSystemService( TJContext.JavaClass.LOCATION_SERVICE );
