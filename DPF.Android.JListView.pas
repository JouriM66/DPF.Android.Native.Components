...
{$IFDEF ANDROID}
  ...
  CallInUIThreadAndWaitFinishing(
    ...
    begin
      ...
      FJDPFListView := TJDPFListView.JavaClass.init( TAndroidHelper.Activity );
      ...
      // FJTextView := TJTextView.JavaClass.init( TAndroidHelper.Activity );
...
begin
  ...
  begin
    ReUsable := false;
    if assigned( cellView ) then begin
      I := cellView.getId;
      ReUsable := FDPFJListView.CellDicCustomViews.ContainsKey( I );
      if ReUsable then
        FDPFJListView.CellDicCustomViews.TryGetValue( I, CustomViews );
    end;
    //I := FDPFJListView.CellDicCustomViews.Count;
  ...
  (* For Test
    ...
    begin
    r := TJLinearLayout.JavaClass.init( TAndroidHelper.Activity );
