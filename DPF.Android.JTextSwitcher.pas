...
{$IFDEF ANDROID}
  CallInUIThreadAndWaitFinishing(
    procedure( )
    begin
      FJTextSwitcher := TJTextSwitcher.JavaClass.init( TAndroidHelper.Activity, nil );
    end );
...
begin
  ...
  FJTextView1 := TJTextView.JavaClass.init( TAndroidHelper.Activity );
  ...
  FJTextView2 := TJTextView.JavaClass.init( TAndroidHelper.Activity );
  ...
  if FAnimationType = atFade then
    AnimIn := TJAnimationUtils.JavaClass.loadAnimation( TAndroidHelper.Activity, android_R_anim_fade_in )
  else
    AnimIn := TJAnimationUtils.JavaClass.loadAnimation( TAndroidHelper.Activity, android_R_anim_slide_in_left );
  AnimIn.setDuration( 1000 );

  if FAnimationType = atFade then
    Animout := TJAnimationUtils.JavaClass.loadAnimation( TAndroidHelper.Activity, android_R_anim_fade_out )
  else
    Animout := TJAnimationUtils.JavaClass.loadAnimation( TAndroidHelper.Activity, android_R_anim_slide_out_right );
